...
ChordSymbolSettingsModel::ChordSymbolSettingsModel(QObject* parent, IElementRepositoryService* repository)
    : AbstractInspectorModel(parent, repository), QAbstractListModel(parent)
{
...
int ChordSymbolSettingsModel::rowCount(const QModelIndex&) const
{
    return m_styles.count();
}

QHash<int, QByteArray> ChordSymbolSettingsModel::roleNames() const
{
...
QVariant ChordSymbolSettingsModel::data(const QModelIndex& index, int role) const
{
...
int ChordSymbolSettingsModel::currentStyleIndex() const
{
    return m_currentStyleIndex;
}

void ChordSymbolSettingsModel::setStyleR(Ms::Sid id, qreal val)
{
    globalContext()->currentNotation()->style()->setStyleValue(id, val);
}

void ChordSymbolSettingsModel::setStyleB(Ms::Sid id, bool val)
{
    globalContext()->currentNotation()->style()->setStyleValue(id, val);
}

void ChordSymbolSettingsModel::initCurrentStyleIndex()
{
    int index = 0;
...
void ChordSymbolSettingsModel::setChordStyle(int index)
{
    m_currentStyleIndex = index;
...
void ChordSymbolSettingsModel::setQualitySymbolsOnStyleChange()
{
...
void ChordSymbolSettingsModel::setPropertiesOnStyleChange()
{
    QString currentStyle = m_styles[m_currentStyleIndex].styleName;
...
void ChordSymbolSettingsModel::setChordSpelling(QString newSpelling)
{
...
void ChordSymbolSettingsModel::extractSelectionHistory(QString selectionHistory)
{
    if (selectionHistory == "") {
        return;
    }
